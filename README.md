...
A GitHub Actions workflow that builds a custom Linux kernel for WSL2 by merging latest linux-stable source with Microsoft's WSL2 kernel config (arch/x86/configs/config-wsl from microsoft/WSL2-Linux-Kernel).
...
msft_branch	linux-msft-wsl-6.18.y	Branch of microsoft/WSL2-Linux-Kernel to pull config-wsl from
linux_branch	linux-rolling-stable	Branch of git.kernel.org/.../linux.git to build
build_deb	false	Set to true to also produce installable .deb packages via make bindeb-pkg
Output
